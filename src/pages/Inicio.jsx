...
const Inicio = () => {
  ...
  try {
    ...
    console.log("RESPUESTA HISTORIAL:", todasLasVentas)
    console.log("Fechas recibidas:", todasLasVentas.map(v => v.fecha))

    const hoy = new Date().toDateString()

    const ventasDelDia = todasLasVentas.filter((venta) => {
      // Convertimos "YYYY-MM-DD" a "YYYY/MM/DD" para que Date lo parsee en local
          ...
          <div className="section-header">
            ...
            <div className="section-actions">
              <span className="ventas-count">
                {ventasHoy.length} venta{ventasHoy.length !== 1 ? "s" : ""} registrada
                {ventasHoy.length !== 1 ? "s" : ""}
              </span>
            </div>
          </div>
        ...
        <div className="footer-section">
          <div className="footer-content">
            <p className="footer-text">
              💡 <strong>Tip:</strong> Los datos se actualizan automáticamente. Usa el botón "Actualizar" para refrescar
              manualmente.
            </p>
          </div>
        </div>
